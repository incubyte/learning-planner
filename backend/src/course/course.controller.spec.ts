...
describe('CourseController', () => {
  ...
  it('should filter popular course by tags which is provided', async () => {
    const courses: CourseDto[] = [
      {
        name: 'Day 1 clean code kata',
        resourseUrls: [
    ...
    tags.push('clean-code', 'refactoring');
    jest
      .spyOn(service, 'filterPopularCourseByTags')
      .mockResolvedValueOnce(mockResponse);
    const result = await controller.filterPopularCourseByTags(tags);
    expect(service.filterPopularCourseByTags).toBeCalledTimes(1);
    expect(result).toMatchObject(mockResponse);
  });
